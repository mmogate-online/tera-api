 ...
 *
 * Wiring is via the routes.adminPanel.admin hook (admin.routes.js), which hands
 * us (router, mod, modules). mod is the admin-server module bag ({ ...modules,
 * i18n, passport, app }); the per-server Express app and passport instance live
 * on mod, NOT on the third `modules` arg (that is the global bag without `app`).
 * The only core touch-points are the login-view button (guarded by
 * res.locals.oidcEnabled) and a small lazy branch in admin.controller.js
 * logoutAction that calls mod.oidcLogout for RP-initiated logout.
 *
 * Break-glass guarantee: the whole setup runs inside a try/catch so that ANY
 * initialization failure (missing config, Keycloak unreachable, library error)
 * logs and disables OIDC rather than crashing the Admin Panel, keeping the QA
 * credential usable when Keycloak is down.
 *
...
/**
 * Wires the OIDC strategy, routes, login-button locals and RP-logout helper.
 * Throws on any failure; the caller catches and degrades to QA break-glass.
 *
 * @param {import("express").Router} router
 * @param {modules & { passport: import("passport").Authenticator, app: import("express").Application }} mod
 * @param {import("../../utils/logger")} logger
 */
const setupOidc = async (router, mod, logger) => {
	...
	// App-level middleware (registered during plugin load, before the router is
	// mounted) so the GET /login view can render the Keycloak button. The app and
	// passport live on `mod`, not on the global `modules` bag.
	mod.app.use((req, res, next) => {
	...
	// Begin the Authorization Code + PKCE flow (strategy builds the authz URL
	// with PKCE S256 + state + nonce and 302s to Keycloak).
	router.get("/login/oidc", mod.passport.authenticate("oidc"));

	// Registered redirect URI. The strategy runs authorizationCodeGrant,
	// validating the authorization response and the ID token before the verify
	// callback fires.
	...
	routes: {
		adminPanel: {
			...
			admin: async (router, mod, modules) => {
				const logger = mod.logger || modules.logger;
				...
				try {
					await setupOidc(router, mod, logger);
				} catch (err) {
					logger.error(
						"OIDC: initialization failed; OIDC stays disabled and QA break-glass " +
						`login remains available. ${err && err.stack ? err.stack : err}`
					);
				}
			}
		}
